...
def test_banana_image_recognition_with_different_temperatures():
    ...
    for temperature in temperatures:
        ...
        assert response is not None, f"Response should not be None for temperature {temperature}"
        assert len(response) > 0, f"Response should not be empty for temperature {temperature}"
        assert "banana" in response.lower(), (
