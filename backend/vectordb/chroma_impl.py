...
class ChromaVectorDB(VectorDBProtocol):
    def __init__(
        self, persist_path: str = "./chroma_db", collection_name: str = "docs"
    ):
    ...
    def search(self, query_vector: List[float], limit: int = 5) -> List[Dict[str, Any]]:
        ...
        if results["ids"]:
            ids = results["ids"][0]
            distances = (
                results["distances"][0] if "distances" in results else [0] * len(ids)
            )
            metas = (
                results["metadatas"][0] if "metadatas" in results else [{}] * len(ids)
            )
            ...
            for i, uid in enumerate(ids):
                ...
                parsed_results.append({"id": uid, "score": score, "metadata": metas[i]})
