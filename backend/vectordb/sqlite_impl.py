...
class SQLiteVectorDB(VectorDBProtocol):
    ...
    def add_chunks(
        ...
        try:
            for i, vector in enumerate(embeddings):
                ext_id = ids[i]
                meta_json = (
                    json.dumps(metadata[i]) if metadata and i < len(metadata) else "{}"
                )

                # 1. Insert into mapping table to get a ROWID
                # We store metadata here so we can retrieve it during search
                cursor.execute(
                    "INSERT OR REPLACE INTO vec_id_map (external_id, metadata_json) VALUES (?, ?)",
                    (ext_id, meta_json),
                )
    ...
    def search(self, query_vector: List[float], limit: int = 5) -> List[Dict[str, Any]]:
        ...
        for rowid, dist in vec_rows:
            res = cursor.execute(
                "SELECT external_id, metadata_json FROM vec_id_map WHERE rowid = ?",
                (rowid,),
            ).fetchone()
            if res:
                ext_id = res[0]
                ...
                results.append(
                    {"id": ext_id, "score": 1.0 - dist, "metadata": meta_dict}
                )
    ...
    def delete(self, ids: List[str]) -> bool:
        ...
        try:
            for eid in ids:
                cursor.execute(
                    "SELECT rowid FROM vec_id_map WHERE external_id = ?", (eid,)
                )
                row = cursor.fetchone()
                if row:
                    rid = row[0]
